...
P = {'bg': '#0d1117', 'panel': '#161b22', 'border': '#30363d', 'text': '#e6edf3', 'sub': '#8b949e', 'grid': '#21262d', 'a1': '#58a6ff', 'a2': '#3fb950', 'a3': '#f78166', 'a4': '#d2a8ff', 'a5': '#ffa657', 'a6': '#79c0ff', 'a7': '#ff7b72', 'a8': '#56d364'}
CMAP_HEAT = 'plasma'
CMAP_PHASE = 'hsv'
CMAP_PROB = 'Blues'
    ...
    if q >= 4:
        ...
        def global_h(local_s):
            ...
            def scan_fn(carry, slice_s):
                ...
                n_slice = jnp.where(bit_val == 0, slice_s + p_slice, p_slice - slice_s) / jnp.sqrt(2.0)
...
def _ctrl_phase_single(state, ctrl, tgt, n_counting, cos_t, sin_t):
    ...
    phase = jnp.where((bit_c == 1) & (bit_t == 1), cos_t + 1j * sin_t, jnp.complex64(1.0))
...
def run_shor_circuit(a: int, N: int, n_counting: int, n_work: int, verbose: bool=True):
    n = n_counting + n_work
    ...
    if verbose:
        print(f'      Done  ({timing['init_s']:.3f}s)')
    if verbose:
    ...
    if verbose:
        print(f'      Done  ({timing['hadamard_s']:.3f}s)')
    ...
    for j, ctrl_q in enumerate(counting_qubits):
        if verbose and (j % 4 == 0 or j == n_counting - 1):
            print(f'      ctrl qubit {ctrl_q:2d}/{n_counting - 1}  a^(2^{j}) mod {N} = {a_pow}', flush=True)
    ...
    if verbose:
        print(f'      Done  ({timing['mod_exp_s']:.3f}s)')
    ...
    if verbose:
        print(f'      Done  ({timing['iqft_s']:.3f}s)')
    ...
    if verbose:
        print(f'  Extracting counting phases ...', flush=True)
    ...
    return (np.array(probs_gathered), state, timing, phase_snapshots, a_pow_sequence, counting_phases)
...
def run_shor_factoring(N: int, a: int, n_counting: int, n_work: int):
    ...
    if g > 1:
        ...
        return {'N': N, 'a': a, 'factor_p': g, 'factor_q': N // g, 'method': 'gcd_trivial', 'success': True, 'timing': {'init_s': 0, 'hadamard_s': 0, 'mod_exp_s': 0, 'iqft_s': 0, 'total_s': 0}, 'r_classical': 0, 'probs': [], 'n_qubits_total': n_counting + n_work, 'n_counting': n_counting, 'n_work': n_work, 'top_measurements': [], 'phase_snapshots': [], 'a_pow_sequence': [], 'counting_phases': []}
    r_classical = classical_order(a, N)
    print(f'  Classical order of {a} mod {N} : r = {r_classical}')
    probs, state, timing, phase_snapshots, a_pow_sequence, counting_phases = run_shor_circuit(a, N, n_counting, n_work, verbose=True)
    ...
    for idx_val, prob in zip(top_indices, top_probs):
        ...
        results_tried.append({'measurement': int(idx_val), 'prob': float(prob), 'r_candidate': r_cand})
    p, q = (None, None)
    if found_factor:
        p, q = found_factor
        print(f'\n  ✅  FACTORS FOUND: {N} = {p} × {q}')
    ...
    return {'N': N, 'a': a, 'n_counting': n_counting, 'n_work': n_work, 'n_qubits_total': n_counting + n_work, 'r_classical': r_classical, 'factor_p': p, 'factor_q': q, 'success': found_factor is not None, 'timing': timing, 'top_measurements': results_tried[:32], 'probs': probs.tolist(), 'phase_snapshots': [(name, snap.tolist()) for name, snap in phase_snapshots], 'a_pow_sequence': a_pow_sequence, 'counting_phases': counting_phases[:2048]}
...
def plot_spectrum(res, n_counting, ts):
    ...
    fig.suptitle(f"⚛  Shor's Algorithm — Measurement Spectrum  │  N={N}, a={a}  │  Period r={r_cl}  │  {BACKEND.upper()}  │  {ts}", color=P['text'], fontsize=13, fontweight='bold', y=0.98)
    ax = axes[0, 0]
    mask = probs_arr > 1e-07
    ax.bar(idx_arr[mask], probs_arr[mask], color=P['a1'], alpha=0.85, width=1.0, edgecolor='none')
    if r_cl > 0:
        peaks = [round(j * counting_dim / r_cl) for j in range(r_cl)]
        for pk in peaks:
            if 0 <= pk < counting_dim:
                ax.axvline(pk, color=P['a3'], lw=0.9, alpha=0.75, ls='--')
        ax.axvline(peaks[0] if peaks else 0, color=P['a3'], lw=0.9, ls='--', label=f'Expected peaks (every {counting_dim // r_cl})')
    ...
    if r_cl > 0 and counting_dim > r_cl:
        ...
        ax3.annotate(f's = {center}\n2ⁿ/r ≈ {counting_dim // r_cl}\n→ r = {r_cl}', xy=(center, zoom_probs.max()), xytext=(center + window // 3, zoom_probs.max() * 0.8), color=P['a5'], fontsize=10, fontweight='bold', arrowprops=dict(arrowstyle='->', color=P['a5'], lw=1.5))
    ...
    if r_cl > 0 and r_cl <= 20:
        ...
        ax4.set_xticklabels([f'j={j}\n({peak_positions[j]})' for j in range(r_cl)], fontsize=8, color=P['text'])
...
def plot_qft_analysis(res, n_counting, ts):
    ...
    fig.suptitle(f'🔬  QFT Phase Analysis  │  N={N}, a={a},  r={r_cl}  │  {ts}', color=P['text'], fontsize=13, fontweight='bold', y=0.98)
    ax = axes[0, 0]
    if phases:
        ph_arr = np.array(phases[:min(2048, len(phases))])
        n_ph = len(ph_arr)
        amp_arr = np.sqrt(probs[:n_ph])
        ax.scatter(np.cos(ph_arr), np.sin(ph_arr), c=amp_arr, cmap='plasma', s=6, alpha=0.7, vmin=0)
    ...
    if phases and r_cl > 0:
        ...
        ax2.plot(np.arange(n_show), np.mod(ideal_phase, 2 * np.pi) - np.pi, color=P['a3'], lw=1.5, ls='--', label=f'Ideal 2π/r ramp (r={r_cl})')
    ax2.set_xlabel('Counting-register index')
    ...
    if a_seq:
        ax3.step(range(len(a_seq)), a_seq, where='mid', color=P['a2'], lw=2.0, label=f'a^(2^j) mod {N}')
        ax3.axhline(1, color=P['a3'], ls='--', lw=1.2, label='1 (identity)')
        ax3.set_xlabel('Counting qubit j')
        ax3.set_ylabel(f'a^(2^j) mod {N}')
        ax3.set_title(f'Mod-Exp Sequence  a={a}, N={N}\n(cycles with period {r_cl} — r detected!)')
        ax3.legend(facecolor=P['panel'], edgecolor=P['border'], labelcolor=P['text'], fontsize=9)
    theme(fig, ax3)
    ax4 = axes[1, 0]
    if r_cl > 0 and counting_dim > r_cl:
        s0 = round(counting_dim / r_cl)
        convergents = continued_fraction_convergents(s0, counting_dim, r_cl * 3)
        qs = [q for _, q in convergents]
        errs = [abs(p / q - s0 / counting_dim) if q else 1.0 for p, q in convergents]
        r_found_idx = next((i for i, (_, q) in enumerate(convergents) if q == r_cl), None)
        ax4.semilogy(range(len(convergents)), [max(e, 1e-14) for e in errs], 'o-', color=P['a4'], lw=2, ms=8, label='|p/q − s/2ⁿ|')
        if r_found_idx is not None:
            ax4.axvline(r_found_idx, color=P['a2'], lw=2, ls='--', label=f'r={r_cl} found at step {r_found_idx}')
            ax4.scatter([r_found_idx], [max(errs[r_found_idx], 1e-14)], color=P['a2'], s=120, zorder=5)
    ...
    if top_ms:
        ...
        for i, (bar, idx_v, rc) in enumerate(zip(bars, ms_idx, ms_r)):
            ...
            ax5.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + max(ms_probs) * 0.01, label, ha='center', va='bottom', color=P['text'], fontsize=6.5)
        ...
        ax5.legend(handles=[Patch(color=P['a2'], label='Period found'), Patch(color=P['a1'], label='No period')], facecolor=P['panel'], edgecolor=P['border'], labelcolor=P['text'], fontsize=9)
    theme(fig, ax5)
    ax6 = axes[1, 2]
    ax6.axis('off')
    ax6.set_facecolor(P['panel'])
    p_, q_ = (res.get('factor_p'), res.get('factor_q'))
    tree_text = f'  Quantum Phase Estimation\n  ─────────────────────────\n  Input  :  N = {N}\n  Base   :  a = {a}\n  Circuit:  {n_counting} counting qubits\n           +{n_work} work qubits\n           = {n_counting + n_work} qubits total\n\n  ↓ Inverse QFT\n\n  Measurement ≈ j · 2²²/r\n  Continued fractions → r\n\n  Period  :  r = {r_cl}\n  r even? :  {('✅ Yes' if r_cl % 2 == 0 else '❌ No')}\n  a^(r/2) :  {(mod_pow(a, r_cl // 2, N) if r_cl % 2 == 0 else 'N/A')}\n\n  gcd(a^(r/2)±1, N):\n  → p = {(p_ if p_ else '?')}\n  → q = {(q_ if q_ else '?')}\n\n  {('✅  N = ' + str(p_) + ' × ' + str(q_) if p_ and q_ else '⚠️  Retry needed')}\n'
    ax6.text(0.05, 0.95, tree_text, transform=ax6.transAxes, color=P['text'], fontsize=10.5, va='top', family='monospace', bbox=dict(facecolor=P['bg'], edgecolor=P['border'], boxstyle='round,pad=0.5'))
...
def plot_circuit_analytics(all_results, n_counting, n_work, ts):
    ...
    gs = gridspec.GridSpec(3, 3, figure=fig, hspace=0.52, wspace=0.4, left=0.06, right=0.97, top=0.92, bottom=0.06)
    fig.suptitle(f"⚙️  Shor's Circuit Analytics  │  {n_counting}+{n_work} = {n} Qubits  │  {NUM_DEV} × TPU v5litepod  │  {ts}", color=P['text'], fontsize=13, fontweight='bold', y=0.97)
    ...
    for i, r in enumerate(all_results):
        ax0.text(i, r['timing']['total_s'] + 0.05, f'{r['timing']['total_s']:.1f}s', ha='center', color=P['text'], fontsize=10, fontweight='bold')
    ax0.set_xticks(x_pos)
    ax0.set_xticklabels(labels_r, color=P['text'], fontsize=10)
    ...
    r0 = all_results[0]
    pie_sizes = [r0['timing']['init_s'], r0['timing']['hadamard_s'], r0['timing']['mod_exp_s'], r0['timing']['iqft_s']]
    pie_labels = ['Init', 'H⊗ⁿ', 'Mod-Exp', 'IQFT']
    pie_colors = [P['a1'], P['a2'], P['a3'], P['a4']]
    wedges, texts, autotexts = ax1.pie(pie_sizes, labels=pie_labels, colors=pie_colors, autopct='%1.1f%%', startangle=140, textprops={'color': P['text'], 'fontsize': 10}, wedgeprops={'edgecolor': P['border'], 'linewidth': 1})
    for at in autotexts:
        at.set_color(P['bg'])
        at.set_fontweight('bold')
    ax1.set_title(f'Time Distribution (N={r0['N']})\nTotal: {r0['timing']['total_s']:.2f}s', color=P['text'])
    ax1.set_facecolor(P['panel'])
    ax2 = fig.add_subplot(gs[0, 2])
    n_h_gates = n_counting
    n_ctrl_mod = n_counting
    n_iqft_h = n_counting
    n_iqft_phase = n_counting * (n_counting - 1) // 2
    n_iqft_swap = n_counting // 2
    ...
    bars = ax2.barh(gate_labels, gate_counts, color=gate_colors, alpha=0.9, edgecolor=P['border'], height=0.6)
    for bar, cnt in zip(bars, gate_counts):
        ax2.text(cnt + max(gate_counts) * 0.01, bar.get_y() + bar.get_height() / 2, str(cnt), va='center', color=P['text'], fontsize=10, fontweight='bold')
    ...
    ax3.axvline(n, color=P['a2'], ls='-.', lw=2.2, label=f'This run: {n}q = {fmt_bytes((1 << n) * 8)}')
    ...
    for row in range(4):
        for col in range(4):
            chip_id = row * 4 + col
            rect = Rectangle((col + 0.05, row + 0.05), 0.9, 0.9, facecolor=P['a1'], edgecolor=P['border'], linewidth=1.5, alpha=0.75 if chip_id < NUM_DEV else 0.15)
            ax4.add_patch(rect)
            ax4.text(col + 0.5, row + 0.5, f'#{chip_id}\n{sv_per_chip_gb:.1f}GB' if chip_id < NUM_DEV else '—', ha='center', va='center', fontsize=8, color=P['bg'] if chip_id < NUM_DEV else P['sub'], fontweight='bold')
    ax4.set_xticks([])
    ax4.set_yticks([])
    ax4.set_title(f'TPU v5litepod-16 Chip Layout\n({NUM_DEV} chips, {sv_per_chip_gb:.2f} GB state-vector / chip)')
    ax4.set_facecolor(P['panel'])
    ax5 = fig.add_subplot(gs[1, 2])
    ax5.axis('off')
    ax5.set_facecolor(P['panel'])
    reg_text = f'  Qubit Register Layout\n  ══════════════════════\n\n  Qubits  0 – {n_counting - 1:2d}  │  Counting Register\n  ({n_counting} qubits, 2²² = {1 << n_counting:,} outcomes)\n\n  Qubits {n_counting} – {n_counting + n_work - 1:2d}  │  Work Register\n  ({n_work} qubits, holds x ∈ [0, N-1])\n\n  Total  :  {n} qubits\n  Dim    :  2^{n} = {1 << n:,}\n  Memory :  {fmt_bytes((1 << n) * 8)}\n\n  ── Gate Complexity ──\n  Hadamard      :  O(n) = {n_counting}\n  Mod-Mul       :  O(n) = {n_counting}\n  IQFT          :  O(n²) = {n_counting ** 2}\n  Total gates   :  ~{n_counting + n_counting + n_counting ** 2:,}\n\n  ── Precision ──\n  2ⁿ = {counting_dim:,}\n  Period resolution: 1/{counting_dim:,}\n  Max N factorable:  {2 ** n_work}\n'
    counting_dim_val = 1 << n_counting
    ax5.text(0.04, 0.97, reg_text, transform=ax5.transAxes, color=P['text'], fontsize=9.5, va='top', family='monospace', bbox=dict(facecolor=P['bg'], edgecolor=P['border'], boxstyle='round,pad=0.5'))
    ...
    for ri, res in enumerate(all_results):
        a_seq = res['a_pow_sequence']
        if a_seq:
            ax6.plot(range(len(a_seq)), a_seq, 'o-', color=colors_seq[ri % len(colors_seq)], lw=2, ms=5, label=f'N={res['N']}, a={res['a']}, r={res['r_classical']}')
    ...
    ax7.axhline(ideal_entropy, color=P['a3'], ls='--', lw=1.5, label=f'Max entropy = {ideal_entropy} bits\n(uniform distribution)')
    ax7.axhline(math.log2(max((r['r_classical'] for r in all_results)) + 1), color=P['a2'], ls=':', lw=1.5, label='Ideal Shor entropy ≈ log₂(r)')
...
def plot_master_dashboard(all_results, n_counting, n_work, ts):
    ...
    gs = gridspec.GridSpec(4, 3, figure=fig, hspace=0.55, wspace=0.38, left=0.05, right=0.97, top=0.93, bottom=0.04)
    fig.suptitle(f"⚛  Shor's Algorithm — 33-Qubit Full State-Vector Master Dashboard\n{BACKEND.upper()}  │  {NUM_DEV} × TPU v5litepod-16  │  {n_counting} counting + {n_work} work qubits  │  {fmt_bytes((1 << n) * 8)} state vector  │  {ts}", color=P['text'], fontsize=12, fontweight='bold', y=0.97)
    for ri, res in enumerate(all_results):
    ...
    for i, r in enumerate(all_results):
        ax4.text(i, r['timing']['total_s'] + 0.02, f'{r['timing']['total_s']:.1f}s', ha='center', color=P['text'], fontsize=9, fontweight='bold')
    ax4.set_xticks(x_pos)
    ax4.set_xticklabels(labels_r, color=P['text'])
    ax4.set_ylabel('Time (s)')
    ax4.set_title('Circuit Timing Breakdown')
    ...
    ax5.set_xticks(x_p)
    ax5.set_xticklabels(labels_r, color=P['text'])
    ax5.set_ylabel('Probability')
    ax5.set_title('Peak Probability: Simulated vs Ideal (1/r)')
    ...
    for row in range(4):
        for col in range(4):
            cid = row * 4 + col
            rect = Rectangle((col + 0.05, row + 0.05), 0.9, 0.9, facecolor=P['a1'], edgecolor=P['border'], lw=1.5, alpha=0.75 if cid < NUM_DEV else 0.1)
            ax7.add_patch(rect)
            ax7.text(col + 0.5, row + 0.5, f'#{cid}\n{sv_per_chip:.1f}G' if cid < NUM_DEV else '—', ha='center', va='center', fontsize=7.5, color=P['bg'] if cid < NUM_DEV else P['sub'], fontweight='bold')
    ...
    ax8.axhline(n_counting, color=P['a3'], ls='--', lw=1.5, label=f'Max entropy = {n_counting} bits')
    for i, ent in enumerate(ents):
        ax8.text(i, ent + 0.05, f'{ent:.2f}b', ha='center', color=P['text'], fontsize=10)
    ax8.set_ylabel('Shannon Entropy (bits)')
    ax8.set_title('Measurement Entropy')
    ax8.legend(facecolor=P['panel'], edgecolor=P['border'], labelcolor=P['text'], fontsize=9)
    theme(fig, ax8)
    ax9 = fig.add_subplot(gs[3, :])
    ax9.axis('off')
    col_labels = ['N', 'a', 'Qubits', 'Period r', 'a^(r/2) mod N', 'gcd(a^(r/2)+1,N)', 'gcd(a^(r/2)−1,N)', 'Factors', 'Verified', 'Circuit Time (s)']
    table_data = []
    for res in all_results:
        N_v, a_v = (res['N'], res['a'])
        r_v = res['r_classical']
        ...
        table_data.append([str(N_v), str(a_v), str(res['n_qubits_total']), str(r_v), str(half), str(g1), str(g2), fstr, verify, f'{res['timing']['total_s']:.2f}'])
    tbl = ax9.table(cellText=table_data, colLabels=col_labels, loc='center', cellLoc='center')
    ...
    ax9.set_title("📋  Shor's Factoring Results — Verified Summary", color=P['text'], fontsize=12, fontweight='bold', pad=6)
...
def plot_summary_card(all_results, n_counting, n_work, ts):
    ...
    ax.axis('off')
    header = f"  Shor's Algorithm — 33-Qubit Simulation\n  Google Cloud TPU v5litepod-16  │  {NUM_DEV} chips  │  {fmt_bytes((1 << n) * 8)} state vector"
    ax.text(0.5, 0.97, header, transform=ax.transAxes, color=P['a1'], fontsize=13, fontweight='bold', ha='center', va='top')
    card_w = 0.28
    card_gap = 0.04
    ...
    for ri, (res, x0) in enumerate(zip(all_results, x_starts)):
        ...
        rect = plt.Rectangle((x0, 0.06), card_w, 0.82, transform=ax.transAxes, facecolor=P['panel'], edgecolor=card_color, linewidth=3, clip_on=False)
        ax.add_patch(rect)
        card_text = f'  N = {N_v}\n  a = {a_v}\n  ─────────────────\n  Qubits : {res['n_qubits_total']}\n  Period : r = {r_v}\n  ─────────────────\n  {('✅  FACTORED!' if success else '⚠️  Retry')}\n  {N_v} = {p_} × {q_}\n\n' if success else f'  {N_v} = ?\n\n'
        card_text += f'  a^(r/2) mod N = {(mod_pow(a_v, r_v // 2, N_v) if r_v % 2 == 0 else 'N/A')}\n  gcd(a^(r/2)+1, N) = {(gcd(mod_pow(a_v, r_v // 2, N_v) + 1, N_v) if r_v % 2 == 0 else '—')}\n\n  Time : {res['timing']['total_s']:.2f} s\n  H⊗ⁿ : {res['timing']['hadamard_s']:.2f} s\n  ModExp : {res['timing']['mod_exp_s']:.2f} s\n  IQFT   : {res['timing']['iqft_s']:.2f} s\n'
        ax.text(x0 + 0.01, 0.83, card_text, transform=ax.transAxes, color=P['text'], fontsize=10.5, va='top', family='monospace', clip_on=False)
        if res['probs']:
            peak_p = max(res['probs'])
            ideal = 1.0 / r_v if r_v > 0 else 0
            ax.text(x0 + card_w / 2, 0.1, f'Peak prob: {peak_p:.4f}  (ideal: {ideal:.4f})', transform=ax.transAxes, color=P['sub'], fontsize=9, ha='center', clip_on=False)
    ax.text(0.5, 0.01, f'Run timestamp: {ts}  │  {n_counting} counting + {n_work} work qubits  │  State vector sharded across {NUM_DEV} TPU chips', transform=ax.transAxes, color=P['sub'], fontsize=9, ha='center', va='bottom')
...
def save_results_csv(all_results, ts):
    ...
    fieldnames = ['N', 'a', 'n_qubits_total', 'r_classical', 'factor_p', 'factor_q', 'success', 't_init_s', 't_hadamard_s', 't_modexp_s', 't_iqft_s', 't_total_s', 'peak_prob', 'ideal_peak_prob', 'n_devices', 'backend', 'timestamp']
    rows = []
    for res in all_results:
        r_v = res['r_classical']
        rows.append({'N': res['N'], 'a': res['a'], 'n_qubits_total': res['n_qubits_total'], 'r_classical': r_v, 'factor_p': res.get('factor_p', ''), 'factor_q': res.get('factor_q', ''), 'success': res['success'], 't_init_s': res['timing']['init_s'], 't_hadamard_s': res['timing']['hadamard_s'], 't_modexp_s': res['timing']['mod_exp_s'], 't_iqft_s': res['timing']['iqft_s'], 't_total_s': res['timing']['total_s'], 'peak_prob': max(res['probs']) if res.get('probs') else 0, 'ideal_peak_prob': 1.0 / r_v if r_v > 0 else 0, 'n_devices': NUM_DEV, 'backend': BACKEND, 'timestamp': ts})
...
def print_system_info(n_counting, n_work):
    ...
    assert sv_gb <= total_hbm_gb - 10, f'State vector ({sv_gb:.1f} GB) exceeds usable HBM!'
    print(f'\n  ✅  Memory check passed — {sv_gb:.1f} GB fits within {total_hbm_gb:.0f} GB HBM\n')
...
if __name__ == '__main__':
    ...
    for N_val, a_val in RUNS:
        ...
        chk = {k: v for k, v in res.items() if k not in ('probs', 'phase_snapshots', 'counting_phases')}
    ...
    json.dump({'timestamp': TS, 'backend': BACKEND, 'n_devices': NUM_DEV, 'n_counting': N_COUNTING, 'n_work': N_WORK, 'n_total_qubits': N_COUNTING + N_WORK, 'state_vector_gb': (1 << N_COUNTING + N_WORK) * 8 / (1 << 30), 'runs': export}, open(json_path, 'w'), indent=2)
    ...
    for res in all_results:
        ...
        print(f'    N={res['N']:4d}  a={res['a']:2d}  r={res['r_classical']:3d}  →  {status}  ({res['timing']['total_s']:.2f}s)')
    print(f'\n  Plots  → tpu/plots/')
