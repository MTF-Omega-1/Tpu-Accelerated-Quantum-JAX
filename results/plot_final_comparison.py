...
def main():
    ...
    labels = ['PennyLane Lightning\n(C++ CPU Engine)', 'jax_qsim (Our Sim)\n(Pure JAX - CPU)', 'jax_qsim (Our Sim)\n(JAX CUDA - RTX 2050 GPU)']
    ...
    def autolabel(rects, is_comp=False):
        for rect in rects:
            ...
            ax.annotate(f'{height:.3f}s', xy=(rect.get_x() + rect.get_width() / 2, height), xytext=(0, 5), textcoords='offset points', ha='center', va='bottom', color=color, fontweight='bold', fontsize=10)
    autolabel(rects1, False)
    autolabel(rects2, True)
    ax.set_title('⚛  25-Qubit State Vector Simulation: Hardware-Accelerated Performance', fontsize=15, color='#e6edf3', fontweight='bold', pad=20)
