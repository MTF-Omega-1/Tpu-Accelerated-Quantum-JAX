...
def build_cirq(n):
    ...
    def circuit_fn(params):
        ...
        for _ in range(LAYERS):
            for q in range(n):
                c.append(cirq.ry(params[p_idx])(qubits[q]))
                p_idx += 1
                c.append(cirq.rz(params[p_idx])(qubits[q]))
                p_idx += 1
            for q in range(n - 1):
...
def run_benchmark():
    ...
    frameworks = [('jax_qsim', '#56d364', 'jax_qsim (Differentiable XLA)'), ('cirq', '#ff7b72', 'Cirq (Standard Statevector)')]
    if has_pennylane:
    ...
    ax.set_yscale('log')
    ax.set_title('⚛  Honest Cross-Framework Execution Speed Comparison (Log Scale)', fontsize=14, color='#e6edf3', fontweight='bold', pad=15)
