...
def adam_update(p_arr, g, m, v, t, lr=0.04, b1=0.9, b2=0.999, eps=1e-08):
    t = t + 1
    m = b1 * m + (1.0 - b1) * g
    v = b2 * v + (1.0 - b2) * g ** 2
    mh = m / (1.0 - b1 ** t)
...
def run_experiment():
    ...
    for u, v, w in EDGES:
        ax_graph.plot([coords_x[u], coords_x[v]], [coords_y[u], coords_y[v]], color='#8b949e', lw=1.0 + w * 1.5, alpha=0.7)
        ax_graph.text((coords_x[u] + coords_x[v]) / 2.0, (coords_y[u] + coords_y[v]) / 2.0, f'{w}', color='#ffa657', fontsize=9, fontweight='bold', ha='center')
    ...
    for q in range(NUM_NODES):
        ax_graph.text(coords_x[q], coords_y[q], f'{q}', color='#e6edf3', fontsize=11, fontweight='bold', ha='center', va='center')
    ...
    for bar in bars:
        ...
        ax_cut.text(bar.get_x() + bar.get_width() / 2.0, height + 0.1, f'{height:.3f}', ha='center', va='bottom', color='#e6edf3', fontsize=10)
    ...
    for i, txt in enumerate(approx_ratios):
        ax_ratio.annotate(f'{txt:.2%}', (depths[i], approx_ratios[i]), textcoords='offset points', xytext=(0, 10), ha='center', color='#e6edf3', fontsize=9, fontweight='bold')
    ...
    for spine in ax_ratio.spines.values():
        spine.set_edgecolor('#30363d')
    fig.suptitle('Quantum Approximate Optimization Algorithm (QAOA) Max-Cut — 6-Node Weighted Graph Solver', color='#e6edf3', fontsize=16, fontweight='bold', y=0.98)
